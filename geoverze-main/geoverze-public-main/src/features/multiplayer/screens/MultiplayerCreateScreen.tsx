...
import { createMultiplayerRoom } from "../data/multiplayerRoomApi";
import {
  DEFAULT_MULTIPLAYER_CAPACITY,
  MULTIPLAYER_CAPACITY_OPTIONS,
  type MultiplayerCapacity,
} from "../types";

/** /play/multiplayer/create — pick a quiz, capacity, and create a private room. */
export function MultiplayerCreateScreen() {
  const navigate = useNavigate();
  const { quizzes, loading, error, refetch } = usePublishedQuizzes();
  const [creatingId, setCreatingId] = useState<string | null>(null);
  const [maxPlayers, setMaxPlayers] = useState<MultiplayerCapacity>(DEFAULT_MULTIPLAYER_CAPACITY);

  const createRoom = async (quizId: string) => {
    setCreatingId(quizId);
    try {
      const state = await createMultiplayerRoom(quizId, maxPlayers);
      navigate({
        to: "/play/multiplayer/room",
      ...
      >
        <AnimatedSection>
          ...
          <p className="mt-3 max-w-xl text-[0.9rem] leading-relaxed text-foreground/60">
            Pick the set your room will run and how many seats to open. You will receive a room
            code to share after creation.
          </p>
        </AnimatedSection>
        ...
        <AnimatedSection className="mt-8">
          <div className="game-surface rounded-2xl p-5 sm:p-6">
            <h2 className="flex items-center gap-2 text-[0.95rem] font-semibold tracking-tight text-foreground">
              <Users className="h-4 w-4 text-bronze" strokeWidth={1.9} aria-hidden />
              Room capacity
            </h2>
            <p className="mt-2 text-[0.82rem] leading-relaxed text-foreground/60">
              Minimum three players required to start. Default is six seats.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              {MULTIPLAYER_CAPACITY_OPTIONS.map((size) => (
                <GeoButton
                  key={size}
                  variant={maxPlayers === size ? "solid" : "dark"}
                  size="md"
