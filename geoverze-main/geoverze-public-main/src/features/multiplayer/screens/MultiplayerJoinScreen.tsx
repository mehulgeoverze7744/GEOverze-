...
import { joinMultiplayerRoom } from "../data/multiplayerRoomApi";

const CODE_LENGTH = 6;

/** /play/multiplayer/join — enter a private room code. */
export function MultiplayerJoinScreen() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const state = await joinMultiplayerRoom(normalized);
      navigate({
      ...
      <SectionContainer className="pt-[calc(var(--nav-height)+var(--space-section-sm))] pb-[var(--space-section-sm)]">
        <AnimatedSection className="mx-auto max-w-xl">
          ...
          <p className="mt-3 text-[0.9rem] leading-relaxed text-foreground/60">
            Ask the host for their six-character code. Codes are case-insensitive.
          </p>
