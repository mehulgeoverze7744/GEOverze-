...
export const ApparelMerchProductPage = memo(function ApparelMerchProductPage({
  product,
  spec,
}: {
  ...
  const selectedVariant = colors[variantIndex] ?? colors[0];
  const wishlisted = wishlist.includes(product.id);

  if (!selectedVariant) return null;

  const selectVariant = (variant: ApparelColorVariant) => {
    const next = colors.findIndex((v) => v.id === variant.id);
    if (next >= 0) setVariantIndex(next);
  };

  const stepVariant = (dir: -1 | 1) => {
    setVariantIndex((i) => (i + dir + colors.length) % colors.length);
  };
    ...
    <PageShell>
      ...
      <div className="relative z-10">
            ...
            <div className="min-w-0 space-y-4">
              ...
              <div>
                <AccordionRow title="Description" icon={FileText}>
                  <p>
                    {product.tagline} Front and back print: {spec.printLine}
                  </p>
                </AccordionRow>
        ...
        <Modal
          ...
          description={`Front and back views — ${spec.title}`}
        >
          <img
            src={selectedVariant.image}
