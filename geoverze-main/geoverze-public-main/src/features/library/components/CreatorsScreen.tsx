...
export function CreatorsScreen() {
  const { creators, loading, error } = usePublishedCreators();

  if (error) {
    return (
      <SectionContainer>
        <EmptyState title="Could not load creators" description={error} />
      </SectionContainer>
    );
  }
  ...
  return (
    <SectionContainer>
      ...
      <div className="mb-8 mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {loading ? (
          <p className="text-sm text-foreground/50">Loading creators…</p>
        ) : (
          creators.map((creator) => (
            ...
            >
              ...
              <p className="mt-4 text-[0.68rem] text-foreground/50">{creator.location}</p>
            </Link>
          ))
        )}
      </div>
