...
export function LibraryCard({
  ...
  const stats = (
    <div className="flex items-center gap-4 text-[0.68rem] text-foreground/50">
      ...
      <span className="inline-flex items-center gap-1.5">
        ...
        {compact(article.likes + (liked ? 1 : 0))}
      </span>
      <span className="inline-flex items-center gap-1.5">
        <Bookmark className="h-3 w-3" strokeWidth={1.6} aria-hidden="true" />
        <span className="sr-only">Bookmarks: </span>
        {compact(article.bookmarks + (saved ? 1 : 0))}
      </span>
    </div>
  );
  ...
  return (
    <article
      className={cn(
        "glass-panel surface-gradient group flex h-full flex-col overflow-hidden rounded-2xl transition-all motion-base hover:-translate-y-1 hover:border-bronze/40 hover:shadow-[var(--glow-bronze)] motion-reduce:hover:translate-y-0",
        className,
      )}
    >
